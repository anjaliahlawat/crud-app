<?php

define("VERSION", "2.3.8");



/*******************************************************************************
    CRUD.php by Bill Weinman <http://bw.org/contact/>

    CRUD: Create, Read, Update, Delete --  Database example

    Copyright (c) 2009-2010 The BearHeart Group LLC

    1.1 bw - 2009-04-22 - Updated to work with PHP warnings turned on
    2.0.0 bw -- 2010-08-09 - Support for SQLite
    2.1.0 bw -- 2010-08-17 - Added UDFs for SQLite
    2.2.0 bw -- 2010-08-24 - Public release with support for both SQLite and MySQL
    2.3.0 bw -- 2010-09-04 - Added support for PostgreSQL
    2.2.8 bw -- 2012-01-27 - Public release

*******************************************************************************/



// ***** uncomment for PostgreSQL
// define("DBENGINE", "pgsql");
// define("PGSQLUSER", "sid");
// define("PGSQLPASS", "foo.bar");
// define("PGSQLDB", "client_details");
// *********************************

define("DBENGINE", "mysql");
define("MYSQLUSER", "root");
define("MYSQLPASS", "");
define("MYSQLDB", "softlinkasia");


$client_details = array ('client_name', 'place', 'software_purchased', 'software_date','purchased_order_no','cost','installation_date'
);
$modules_included_alice;
$modules_included_liberty;

$track_fields = array (
    'track_number', 'client_name','duration'
);

$states=array ('Andhra Pradesh','Andaman & Nicobar','Arunachal Pradesh','Assam','Bihar','Bangladesh','Chhattisgarh','Daman & Diu','Delhi','Goa','Gujarat','Himachal Pradesh','Haryana','J&k','Jharkhand','Karnataka','Kerala','Lakshadweep','Manipur','Madhya Pradesh','Maharashtra','Meghalaya','Mizoram','Nagaland','Odissa','Punjab','Pakistan','Rajasthan','Sri Lanka','Telangana','Tripura','Tamil Nadu','Uttarakhand','Uttar Pradesh','West Bengal'
);

$months = array (
    "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"
);


_init();
main();

function main()
{
    if(! isset($_REQUEST['a'])) $_REQUEST['a'] = '';
    jump($_REQUEST["a"]);
}

function _init( )
{
    global $CRUD;

    $CRUD['client_name'] = "CRUD";
    $CRUD['SELF'] = $_SERVER["SCRIPT_NAME"];

    // loose "index.php" if nec (regexes are fugly in php. Feh.)
    $CRUD["SELF"] = preg_replace('/([\\/\\\])index\\.php$/i', '$1', $CRUD["SELF"]); 

    foreach ( array('DBVERSION', 'BUTTONS', 'HIDDENS', 'MESSAGES', 'ERRORS', 'CONTENT', 'PRECONTENT', 'POSTCONTENT','Aclient_name', 'Aplace', 'Asoftware_purchased','Amodules_included_alice','Amodules_included_liberty', 'Asoftware_day','Asoftware_month', 'Asoftware_year','Apurchased_order_no','Acost','Ainstallation_day' ,'Ainstallation_month','Ainstallation_year','Ttrack_number', 'Tclient_name', 'Tduration') as $v )
        $CRUD[$v] = '';
    $modules_included_alice='';
    $modules_included_liberty='';

    switch(DBENGINE) {
        case 'sqlite3':
            try {
                $dbh = new PDO('sqlite:' . DBFILE, 'unused', 'unused');
                $dbh->sqliteCreateFunction('SEC_TO_TIME', 'sec_to_time', 1);        // custom functions ...
                $dbh->sqliteCreateFunction('TIME_TO_SEC', 'time_to_sec', 1);
                $dbh->sqliteCreateAggregate('SUM_SEC_TO_TIME',
                    'sum_sec_to_time_step', 'sum_sec_to_time_finalize', 1);
                $CRUD['DBVERSION'] = SQLite3::version();
                $CRUD['DBVERSION'] = 'SQLite version ' . $CRUD['DBVERSION']['versionString'];
            } catch (PDOException $e) {
                error($e->getMessage());
            }
            break;
        case 'mysql':
            // connect to the database (persistent)
            try {
                $dbh = new PDO('mysql:host=localhost;dbname=' . MYSQLDB, MYSQLUSER, MYSQLPASS,array( PDO::ATTR_PERSISTENT => true ));
                $sth = $dbh->query("SHOW VARIABLES WHERE Variable_name = 'version'");
                $CRUD['DBVERSION'] = 'MySQL server version ' . $sth->fetchColumn(1);
            } catch (PDOException $e) {
                error($e->getMessage());
            }
            break;
        case 'pgsql':
            // connect to the database (persistent)
            try {
                $dbh = new PDO('pgsql:host=localhost;port=5432;dbname=' . PGSQLDB, PGSQLUSER, PGSQLPASS,
                    array( PDO::ATTR_PERSISTENT => true ));
                $dbh->exec("set client_encoding to 'latin1'");
                $sth = $dbh->query('SELECT VERSION()');
                $CRUD['DBVERSION'] = explode(' ', $sth->fetchColumn());
                $CRUD['DBVERSION'] = 'PostgreSQL server version ' . $CRUD['DBVERSION'][1];
            } catch (PDOException $e) {
                error($e->getMessage());
            }
            break;
        default:
            error('unsupported DBENGINE specified: ' . DBENGINE);
    }
    $CRUD['dbh'] = $dbh;
}

function page( $p )
{
    global $CRUD;   // used in the required files
    if( ! $p ) $p = "main";

    set_vars();

    require_once "assets/header.php";
    require_once "assets/$p.php";
    require_once "assets/footer.php";
    exit();
}

function jump( $action )
{
    switch($action) {
        case "add_client_details":
            do_add_client_details();
            break;
        case "client_details_action":
            do_client_details_action();
            break;
        case "client_details_update":
            do_client_details_update_action();
            break;
        case "track_add":
            do_track_add();
            break;
        case "track_update":
            do_track_update();
            break;
        case "client_details_delete_confirm":
            delete_client_details();
            break;
        case "track_delete_confirm":
            delete_track();
            break;
        default:    // default to show main page
            main_page();
    }
    return;
}

function main_page()
{
    
    form_head('Add client details');
    javascript_focus( 'client_details', 'Aclient_name' );
    hidden('a', 'add_client_details');
    button('add_client_details', ' Add client details ');
    button('get_client_details','Display Clients');
    page('main');
}

// client_details functions

function list_client_detailss()
{
    global $client_details;

    $sth = get_client_detailss_sql();

    // $a is an accumulator for the output string
    $a = subheading('client details');
    $a .= "<table class=\"results\">\n";

    // get the first row
    $row = $sth->fetch(PDO::FETCH_ASSOC);

    if( ! $row ) {
        message("There are no clients in the database. Add some!");
        return;
    }

    $col_names = array_keys($row);

    // head of table
    $a .= "<tr>";   // table row for headings
    foreach( $client_details as $name ) {
        $name = display_name($name);  // uppercase the first char
        $a .= "<td class=\"column_head\">$name</td>\n";
    }

    // space for the links at the right side of the results table
    $a .= "<td class=\"column_head\">Action</td>\n";

    $row_count = 0;
    do {
        $a .= client_details_result_row($row);
        $row_count++;
    } while ( $row = $sth->fetch(PDO::FETCH_ASSOC) );

    $a .= "</tr>\n"; 
    $a .= "</table>\n"; 

    message( "There are only " . number_format($row_count) . " client in the database. Add some more!");
    content($a);
}

function display_tracks ( $client_details_id ) 
{
    global $track_fields;
    
    // $a is an accumulator for the output string
    $a = subheading('Tracks');
    $a .= "<table class=\"results\">\n";

    // head of table
    $a .= "<tr>";   // table row for headings
    foreach( $track_fields as $name ) {
        $name = display_name($name);  // uppercase the first char
        $a .= "<td class=\"column_head\">$name</td>\n";
    }

    // space for the links at the right side of the results table
    $a .= "<td class=\"column_head\">Action</td>\n";

    $a .= new_track($client_details_id);
    $sth = get_tracks_sql($client_details_id);

    $row_count = 0;
    while ( $row = $sth->fetch(PDO::FETCH_ASSOC) ) {
        $a .= track_result_row($row);
        $row_count++;
    }

    $a .= "</table>\n";

    content($a);
}

function client_details_result_row( $row )
{
    global $client_details;     
    $a = "<tr>\n";
    foreach( $client_details as $v ) {
        if( $v == 'client_name' ) {
            $row[$v] .= "<span class=\"small light\">&nbsp;&nbsp;$len</span>";
        }
        $a .= "<td class=\"cell_value\">" . $row[$v] . "</td>\n";
    }
    $a .= client_details_action_buttons( $row['client_name'] );   // the action links for this row
    $a .= "</tr>\n";
    return $a;
}

function client_details_action_buttons( $id )
{
    global $CRUD;
    $link_back = $CRUD['SELF'];

    $a = "<td class=\"cell_value\">";
    $a .= start_form() .
        action_button('edit', 'Edit') .
        action_button('delete', 'Delete') .
        hidden_element('a', 'client_details_action') .
        hidden_element('id', $id) .
        end_form();
    $a .= "</td>";
    return $a;
}

function client_details_edit( $id )
{
    global $CRUD, $client_details;
    $client_details = fetch_client_details($id);  // get the client_details from the database
    if(! $client_details) {
        message("client_details not found.");
        main_page();
    }

    foreach ( $client_details as $f ) {
        if($f == 'software') {
            $CRUD[ 'A' . $f ] = fill_client_details_software_date($client_details[$f]);
        }elseif($f == 'installation'){
            $CRUD[ 'A' . $f ] = fill_client_details_installation_date($client_details[$f]);
        }
       
         else {
            $CRUD[ 'A' . $f ] = $client_details[$f];
        }
    }
    form_head('Edit client_details');
    button('client_details_update', ' Update ');
    button('done', ' Done ');
    hidden('a', 'client_details_update');
    hidden('id', $id);

    display_tracks( $id );

    page('main');
}

function client_details_delete_confirm( $id )
{
    $client_details = fetch_client_details( $id );
    $a = start_form();
    $a .= heading('Confirm Delete');
    $a .= "<p> Are you sure you want to delete the client_details \"" .
        $client_details['client_name'] .
        "\" and all its tracks?" . 
        "</p>\n";
    $a .= "<p>\n";
    $a .= button_element('cancel', " Cancel ") . "&nbsp;";
    $a .= button_element('delete_confirm', " Confirm Delete ");
    $a .= "</p>\n";
    $a .= hidden_element('a', 'client_details_delete_confirm') . hidden_element('id', $client_details['id']);
    $a .= end_form();
    content($a);
    page('plain');
}

function track_delete_confirm( $track_id, $client_details_id )
{
    $client_details = fetch_client_details( $client_details_id );
    $track = fetch_track( $track_id );

    $a = start_form();
    $a .= heading('Confirm Delete');
    $a .= "<p> Are you sure you want to delete the track \"" .
        $track['client_name'] .
        "\" from the client_details \"" .
        $client_details['client_name'] .
        "\"?" . 
        "</p>\n";
    $a .= "<p>\n";
    $a .= button_element('cancel', " Cancel ") . "&nbsp;";
    $a .= button_element('delete_confirm', " Confirm Delete ");
    $a .= "</p>\n";
    $a .= hidden_element('a', 'track_delete_confirm') . hidden_element('id', $track['id']);
    $a .= end_form();
    content($a);
    page('plain');
}

function do_client_details_action()
{
    $id = $_REQUEST['id'];
    if(isset($_REQUEST['edit'])) client_details_edit($id);
    if(isset($_REQUEST['delete'])) client_details_delete_confirm($id);
}

function do_client_details_update_action()
{
    $id = $_REQUEST['id'];
    if(isset($_REQUEST['client_details_update'])) do_update_client_details();
    if(isset($_REQUEST['done'])) main_page();
    main_page();
}

function do_update_client_details()
{
    global $client_details;

    foreach( $client_details as $f ) {
        $fieldname = 'A' . $f;
        if ($f == "released") {
            $client_details[$f] = get_client_details_software_date();
        } elseif ($_REQUEST[$fieldname]) {
            $client_details[$f] = get_field($fieldname);
        }
    }
    $client_details['id'] = $_REQUEST['id'];

    update_client_details_sql( $client_details );

    $client_name = $client_details['client_name'];
    message("client_details \"$Clien_Name\" updated.");
    client_details_edit($client_details['id']);
}

function do_add_client_details()
{
    global $CRUD, $client_details,$modules_included_liberty,$modules_included_alice;

    foreach( $client_details as $f ) {
        $fieldname = 'A' . $f;
        if ($f == "software_date"){
            $client_details[$f] = get_client_details_software_date($fieldname);

        }
        elseif($f == "installation_date"){
           $client_details[$f] = get_client_details_installation_date($fieldname);
        }
        elseif ($_REQUEST[$fieldname]){
            $client_details[$f] = get_field($fieldname);
        }
    }
    $modules_included_liberty = implode(",",$_POST['Liberty']);
    $modules_included_alice = implode(",",$_POST['Alice']);

    if( ! isset( $client_details['client_name'] ) ) error_message( "This field is required1" );
    if( ! isset( $client_details['place'] ) ) error_message( "This field is required2");
    if( ! isset( $client_details['software_purchased'] ) ) error_message( "This field is required3");
    
    if( ! isset( $client_details['software_date'] ) ) error_message( "This field is required4");
    if( ! isset( $client_details['purchased_order_no'] ) ) error_message("This field is required5");
    if( ! isset( $client_details['cost'] ) ) error_message( "This field is required6");
    if( ! isset( $client_details['installation_date'] ) ) error_message( "This field is required7");
    if( error_message() ) {
        foreach ( $client_details as $f ) {
            $CRUD[ 'A' . $f ] = isset($client_details[$f]) ? $client_details[$f] : '';
            if( $f == 'software' and isset($client_details[$f]) ) fill_client_details_software_date($client_details[$f]);
            elseif( $f == 'installation' and isset($client_details[$f]) )
                fill_client_details_installation_date( $client_details[$f] );
        }
        main_page();
    }
    create_client_details( $client_details, $modules_included_alice, $modules_included_liberty);
    main_page();
}
function display_clients()
{
    list_client_detailss();
}

function create_client_details( $client_details, $modules_included_alice, $modules_included_liberty )
{
    $id = insert_client_details_sql( $client_details, $modules_included_alice, $modules_included_liberty );

    $client_name = $client_details['client_name'];
    message("Client  \"$client_name\" added.");
 //   message("You may now add tracks below.");

 //   javascript_focus( 'add_track', 'Ttrack_number' );
 //   client_details_edit( $id );
}

function delete_client_details()
{
    $id = $_REQUEST['id'];
    $client_details = fetch_client_details( $id );
    $client_name = $client_details['client_name'];
    if(isset($_REQUEST['cancel'])) {
        message("Cancelled delete of client_details \"$client_name\"");
        main_page();
    }

    delete_client_details_sql($id);

    message("client_details \"$client_name\" deleted.");
    main_page();
}

function client_details_month_select1()
{
    global $CRUD, $months;
    $a = "<select name=\"Asoftware_month\">";
    $a .= "<option value=\"0\">-- Select a Month --</option>\n";
    for ( $i = 1; $i <= 12; $i++ ) {
        $m = $months[$i - 1];
        $selected = ( sprintf("%02d", $i) == $CRUD['Asoftware_month'] ) ? " selected" : "";
        $a .= "<option value=\"$i\"$selected>$m</option>\n";
        }
    $a .= "</select>\n";
    echo($a);
}
function client_details_month_select2()
{
    global $CRUD, $months;
    $a = "<select name=\"Ainstallation_month\">";
    $a .= "<option value=\"0\">-- Select a Month --</option>\n";
    for ( $i = 1; $i <= 12; $i++ ) {
        $m = $months[$i - 1];
        $selected = ( sprintf("%02d", $i) == $CRUD['Ainstallation_month'] ) ? " selected" : "";
        $a .= "<option value=\"$i\"$selected>$m</option>\n";
        }
    $a .= "</select>\n";
    echo($a);
}
function client_details_place_select()
{
    global $CRUD, $states;
    $a = "<select name=\"Aplace\">";
    $a .= "<option value=\"0\">-- Select a place --</option>\n";
    for ( $i = 1; $i <= 35; $i++ ) {
        $s = $states[$i - 1];
        $selected = ( sprintf("%02d", $i) == $CRUD['Aplace'] ) ? " selected" : "";
        $a .= "<option value=\"$i\"$selected>$s</option>\n";
        }
    $a .= "</select>\n";
    echo($a);
}

function fill_client_details_software_date( $f )
{
    global $CRUD;
    list( $year, $month, $day ) = explode('-', $f, 3);

    $CRUD['Asoftware_year'] = $year;
    $CRUD['Asoftware_day'] = $day;
    $CRUD['Asoftware_month'] = $month;
}
function fill_client_details_installation_date( $f )
{
    global $CRUD;
    list( $year, $month, $day ) = explode('-', $f, 3);

    $CRUD['Ainstallation_year'] = $year;
    $CRUD['Ainstallation_day'] = $day;
    $CRUD['Ainstallation_month'] = $month;
}
function get_client_details_software_date($field)
{
    
    $year = get_field('Asoftware_year');
    $month = get_field('Asoftware_month');
    $day = get_field('Asoftware_day');

    // make sure they're numeric
    if( ! is_numeric($year) ) $year = 0;
    if( ! is_numeric($month) ) $month = 0;
    if( ! is_numeric($day) ) $day = 0;

    if( $year < 1 ) return NULL;
    if( $month < 1 ) $month = 1;
    if( $day < 1 ) $day = 1;

    // an SQL date looks like: "2009-01-24"
    return sprintf("%04d-%02d-%02d", $year, $month, $day);
}

function get_client_details_installation_date($field)
{
    
    $year = get_field('Ainstallation_year');
    $month = get_field('Ainstallation_month');
    $day = get_field('Ainstallation_day');

    // make sure they're numeric
    if( ! is_numeric($year) ) $year = 0;
    if( ! is_numeric($month) ) $month = 0;
    if( ! is_numeric($day) ) $day = 0;

    if( $year < 1 ) return NULL;
    if( $month < 1 ) $month = 1;
    if( $day < 1 ) $day = 1;

    // an SQL date looks like: "2009-01-24"
    return sprintf("%04d-%02d-%02d", $year, $month, $day);
}

// track functions

function do_track_update()
{
    global $CRUD, $track_fields;
    $client_details_id = $_REQUEST['client_details_id'];
    $track_id = $_REQUEST['id'];

    if(isset($_REQUEST['track_delete'])) track_delete_confirm( $track_id, $client_details_id );

    $track = validate_track_input();
    $track['id'] = $track_id;

    update_track_sql($track);

    $track_number = $track['track_number'];
    $client_name = $track['client_name'];
    message("Track $track_number ($client_name) updated.");

    // reset the display variables
    $CRUD['Ttrack_number'] = "";
    $CRUD['Tclient_name'] = "";
    $CRUD['Tduration'] = "";
    client_details_edit( $client_details_id );
}

function do_track_add ()
{
    global $CRUD;
    $client_details_id = $_REQUEST['client_details_id'];

    $track = validate_track_input();
    $track['client_details_id'] = $client_details_id;
    $id = insert_track_sql($track);

    $track_number = $track['track_number'];
    $client_name = $track['client_name'];
    message("Track $track_number ($client_name) added.");

    $CRUD['Ttrack_number'] = "";
    $CRUD['Tclient_name'] = "";
    $CRUD['Tduration'] = "";

    javascript_focus( 'add_track', 'Ttrack_number' );
    client_details_edit( $client_details_id );
}

function validate_track_input ()
{
    global $CRUD;
    $client_details_id = $_REQUEST['client_details_id'];

    $track_number = get_field('Ttrack_number');
    $client_name = get_field('Tclient_name');
    $duration = get_field('Tduration');

    $CRUD['Ttrack_number'] = $track_number;
    $CRUD['Tclient_name'] = $client_name;
    $CRUD['Tduration'] = $duration;

    // check for errors
    if( ! strlen($track_number) ) error_message("A track must have a track number");
    if( ! $client_name ) error_message("A track must have a client_name");
    if( ! $duration ) error_message("A track must have a duration");
    if( preg_match( '/[^0-9:]/', $duration ) )
        error_message("Duration must be in seconds, or minutes and seconds, e.g., \"7:32\"");

    // a little extra checking for duration
    $duration_array = explode(':', $duration );
    $duration_count = count($duration_array);
    if( $duration_count == 1 )
        $db_duration = $duration_array[0];
    elseif( $duration_count == 2)
        $db_duration = ( $duration_array[0] * 60 ) + $duration_array[1];
    else error_message("Duration must be in seconds, or minutes and seconds, e.g., \"7:32\"");

    // report any errors
    if( error_message() ) {
        client_details_edit($client_details_id);
    }

    $track['client_details_id'] = $client_details_id;
    $track['track_number'] = $track_number;
    $track['client_name'] = $client_name;
    $track['duration'] = $db_duration;

    return $track;
}

function new_track ( $client_details_id )
{
    global $CRUD;
    $link_back = $CRUD['SELF'];
    $a = start_form( 'add_track' );
    $a .= "<tr>\n";
    $a .= "<td class=\"cell_value\">" . track_input_text( 'Ttrack_number', 'Ttrack_number', $CRUD['Ttrack_number'] ) . "</td>\n";
    $a .= "<td class=\"cell_value\">" . track_input_text( 'Tclient_name', 'Tclient_name', $CRUD['Tclient_name'] ) . "</td>\n";
    $a .= "<td class=\"cell_value\">" . track_input_text( 'Tduration', 'Tduration', $CRUD['Tduration'] ) . "</td>\n";
    $a .= "<td class=\"cell_value\">" . action_button( 'track_add', ' Add ' ) . "</td>\n";
    $a .= hidden_element( 'a', 'track_add' ) . hidden_element( 'client_details_id', $client_details_id ) . "\n";
    $a .= "</tr>\n";
    $a .= end_form();
    return $a;
}

function track_result_row( $row )
{
    $a = start_form();
    $a .= "<tr>\n";
    $a .= "<td class=\"cell_value\">" . track_input_text( 'Ttrack_number', 'Ttrack_number', $row['track_number'] ) . "</td>\n";
    $a .= "<td class=\"cell_value\">" . track_input_text( 'Tclient_name', 'Tclient_name', $row['client_name'] ) . "</td>\n";
    $a .= "<td class=\"cell_value\">" . track_input_text( 'Tduration', 'Tduration', $row['disp_duration'] ) . "</td>\n";
    $a .= "<td class=\"cell_value\">" . action_button( 'track_update', ' Update ' ) .
        action_button( 'track_delete', ' Delete ' ) . "</td>\n";
    $a .= hidden_element( 'a', 'track_update' ) . hidden_element( 'client_details_id', $row['client_details_id'] ) . hidden_element( 'id', $row['id'] ) . "\n";
    $a .= "</tr>\n";
    $a .= end_form();
    return $a;
}

function delete_track()
{
    global $CRUD;

    $id = $_REQUEST['id'];
    $track = fetch_track( $id );
    $client_name = $track['client_name'];

    if(isset($_REQUEST['cancel'])) {
        message("Cancelled delete of track \"$client_name\"");
        client_details_edit($track['client_details_id']);
    }

    delete_track_sql($id);

    message("Track \"$client_name\" deleted.");
    client_details_edit($track['client_details_id']);
}

//
// database interface functions and SQL
//

// perform the query and return sth (statement handle)
function get_client_detailss_sql ( )
{
    global $CRUD;
    $dbh = $CRUD['dbh'];

    $query = ' SELECT client_name, place, software_purchased, software_date, purchased_order_no, cost, installation_date FROM client_details ORDER BY client_name ';
     $sth = $dbh->prepare($query);
    if($sth) $sth->execute();
    else error('get_client_detailss_sql: select prepare returned no statement handle');

    $err = $sth->errorInfo();
    if($err[0] != 0) error( $err[2] );

    return($sth);
}

// perform the query and return sth (statement handle)
function get_tracks_sql ( $client_details_id )
{
    global $CRUD;
    $dbh = $CRUD['dbh'];

    switch(DBENGINE) {
        case 'mysql':
            $query = '
                SELECT 
                    id, client_details_id, client_name, track_number,
                    CONCAT_WS(
                        ":",
                        duration DIV 60,
                        LPAD( duration MOD 60, 2, "0" )
                    ) AS disp_duration
                    FROM track
                    WHERE client_details_id = ?
                    ORDER BY track_number, id
            ';
            break;
        case 'MySQL':
            $query = "
                SELECT 
                    id, client_details_id, client_name, track_number, SEC_TO_TIME(duration) AS disp_duration
                    FROM track
                    WHERE client_details_id = ?
                    ORDER BY track_number, id
            ";
            break;
        case 'pgsql':
            $query = "
                SELECT 
                    id, client_details_id, client_name, track_number,
                    duration / 60 || ':' || TO_CHAR(duration % 60, 'FM09') AS disp_duration
                    FROM track
                    WHERE client_details_id = ?
                    ORDER BY track_number, id
            ";
            break;
        default:
            error("get_tracks_sql: no SQL for engine " . DBENGINE);
            return NULL;
    }

    $sth = $dbh->prepare($query);
    if($sth) $sth->execute( array( $client_details_id ) );
    else error("get_tracks_sql: select prepare returned no statement handle (" . db_message($dbh) . ")");
    $err = $sth->errorInfo();
    if($err[0] != 0) error( $err[2] );

    return($sth);
}

function insert_client_details_sql( $client_details, $modules_included_alice, $modules_included_liberty )
{
    global $CRUD;
    $dbh = $CRUD['dbh'];

    $query = '
        INSERT INTO client_details
            (client_name, place, software_purchased, modules_included_alice ,modules_included_liberty, software_date, purchased_order_no, cost, installation_date)
            VALUES (:client_name,:place, :software_purchased, :modules_included_alice,:modules_included_liberty,:software_date, :purchased_order_no, :cost,:installation_date)
    ';

    $sth = $dbh->prepare($query);

    if($sth)
    {
        $sth->bindParam(':client_name', $client_details['client_name']);
        $sth->bindParam(':place', $client_details['place']);
        $sth->bindParam(':software_purchased', $client_details['software_purchased']);
        $sth->bindParam(':modules_included_alice', $modules_included_alice);
        $sth->bindParam(':modules_included_liberty', $modules_included_liberty); 
        $sth->bindParam(':software_date', $client_details['software_date']);
        $sth->bindParam(':purchased_order_no', $client_details['purchased_order_no']);
        $sth->bindParam(':cost', $client_details['cost']);
        $sth->bindParam(':installation_date', $client_details['installation_date']);

        $sth->execute();
    } 
    else error("insert_client_details_sql: insert prepare returned no statement handle");

    // check for errors
    $err = $sth->errorInfo();
    if($err[0] != 0) error( $err[2] );

    // this is a kludge to make pgsql work like the others
/*    if(DBENGINE == 'pgsql') {
        $id = $dbh->lastInsertId('client_details_id_seq');
    } else {
        $id = $dbh->lastInsertId();
    }

    return($id);*/
}

function insert_track_sql ( $track )
{
    global $CRUD;
    $dbh = $CRUD['dbh'];

    // database insert
    $query = '
        INSERT INTO track
            ( client_details_id, track_number, client_name, duration )
            VALUES ( ?, ?, ?, ? )
    ';

    $sth = $dbh->prepare($query);
    if($sth) $sth->execute( array( @$track['client_details_id'], @$track['track_number'], @$track['client_name'], @$track['duration']) );
    else error("insert_track_sql: insert prepare returned no statement handle");

    // check for errors from the database ops
    $err = $sth->errorInfo();
    if($err[0] != 0) error( $err[2] );

    // this is a kludge to make pgsql work like the others
    if(DBENGINE == 'pgsql') {
        $id = $dbh->lastInsertId('track_id_seq');
    } else {
        $id = $dbh->lastInsertId();
    }

    return($id);
}

function update_client_details_sql( $client_details )
{
    global $CRUD;
    $dbh = $CRUD['dbh'];

    $query =  '
        UPDATE client_details 
            SET client_name = ?, place = ?, Software Purchased = ?, released = ?
            WHERE id = ?
    ';

    $sth = $dbh->prepare($query);
    if($sth) $sth->execute( array( @$client_details['client_name'], @$client_details['place'], @$client_details['Software Purchased'], @$client_details['released'], @$client_details['id'] ) );
    else error("update_client_details_sql: update prepare returned no statement handle");

    // check for errors
    $err = $sth->errorInfo();
    if($err[0] != 0) error( $err[2] );
}

function update_track_sql ( $track )
{
    global $CRUD;
    $dbh = $CRUD['dbh'];

    $query = '
        UPDATE track
            SET track_number = ?, client_name = ?, duration = ?
            WHERE id = ?
    ';
    $sth = $dbh->prepare($query);
    if($sth) $sth->execute( array( @$track['track_number'], @$track['client_name'], @$track['duration'], @$track['id'] ) );
    else error("update_track_sql: update prepare returned no statement handle");

    // check for errors from the database ops
    $err = $sth->errorInfo();
    if($err[0] != 0) error( $err[2] );
}

function delete_client_details_sql( $id )
{
    global $CRUD;
    $dbh = $CRUD['dbh'];

    $query1 =  "DELETE FROM track  WHERE client_details_id = ?";
    $query2 =  "DELETE FROM client_details  WHERE id = ?";

    // delete tracks
    $sth = $dbh->prepare($query1);
    if($sth) $sth->execute( array( $id ) );
    else error("delete_client_details_sql: delete prepare returned no statement handle");

    // check for errors
    $err = $sth->errorInfo();
    if($err[0] != 0) error( $err[2] );

    // delete client_details
    $sth = $dbh->prepare($query2);
    if($sth) $sth->execute( array( $id ) );
    else error("delete_client_details_sql: delete prepare returned no statement handle");

    // check for errors
    $err = $sth->errorInfo();
    if($err[0] != 0) error( $err[2] );
}

function delete_track_sql ( $id )
{
    global $CRUD;
    $dbh = $CRUD['dbh'];

    $query =  "DELETE FROM track WHERE id = ?";

    // delete track
    $sth = $dbh->prepare($query);
    if($sth) $sth->execute( array( $id ) );
    else error("delete_track: delete prepare returned no statement handle");

    // check for errors
    $err = $sth->errorInfo();
    if($err[0] != 0) error( $err[2] );
}

// "fetch_" functions are for getting one row
function fetch_client_details( $id )
{
    global $CRUD;
    $dbh = $CRUD['dbh'];
    $query = 'SELECT * FROM client_details WHERE id = ?';
    $sth = $dbh->prepare($query);
    if($sth) $sth->execute( array( $id ) );
    else error("fetch_client_details: select prepare returned no statement handle");
    return $sth->fetch(PDO::FETCH_ASSOC);
}

function fetch_track( $id )
{
    global $CRUD;
    $dbh = $CRUD['dbh'];

    switch(DBENGINE) {
        case 'mysql':
            $query = '
                SELECT 
                    id, client_details_id, client_name, track_number,
                    CONCAT_WS(
                        ":",
                        duration DIV 60,
                        LPAD( duration MOD 60, 2, "0" )
                    ) AS disp_duration
                    FROM track
                    WHERE id = ?
            ';
            break;
        case 'MySQL':
            $query = "
                SELECT 
                    id, client_details_id, client_name, track_number, SEC_TO_TIME(duration) AS disp_duration
                    FROM track
                    WHERE id = ?
            ";
            break;
        case 'pgsql':
            $query = "
                SELECT 
                    id, client_details_id, client_name, track_number,
                    duration / 60 || ':' || TO_CHAR(duration % 60, 'FM09') AS disp_duration
                    FROM track
                    WHERE id = ?
            ";
            break;
        default:
            $query = '';
            break;
    }

    $sth = $dbh->prepare($query);
    if($sth) $sth->execute( array( $id ) );
    else error("fetch_track: select prepare returned no statement handle");
    return $sth->fetch(PDO::FETCH_ASSOC);
}

// custom functions for SQLite 3

// SEC_TO_TIME( seconds INTEGER )
function sec_to_time( $sec )
{
    if(is_null($sec)) return NULL;
    $sec = intval($sec);    // make sure it's an integer
    $s = $sec % 60;
    $m = $sec / 60;
    return sprintf('%d:%02d', $m, $s);
}

// TIME_TO_SEC( time TEXT )  -- 'mm:ss'
function time_to_sec( $time )
{
    if(is_null($time)) return NULL;
    $t = explode(':', $time, 2);
    $m = intval($t[0]);
    $s = intval($t[1]);
    return ( $m * 60 ) + $s;
}

// SUM_SEC_TO_TIME
function sum_sec_to_time_step ($context, $rownumber, $value)
{
    if(is_null($value)) return $context;
    if(is_null($context)) $context = 0;
    $context += intval($value);     // accumulate the sum of the values
    return $context;
}

function sum_sec_to_time_finalize ( $context, $rownumber )
{
    $sec = $context;
    $s = $sec % 60;
    $m = $sec / 60;
    $h = 0;
    if($m > 60) {
        $h = $m / 60;
        $m = $m % 60;
    }
    if($h) return sprintf('%d:%02d:%02d', $h, $m, $s);
    else return sprintf('%02d:%02d', $m, $s);
}

/*function get_client_details_length($id)
{
    global $CRUD;
    if(! in_array(DBENGINE, array('MySQL', 'pgsql'))) return NULL;
    $dbh = $CRUD['dbh'];
    $query = '';
    switch(DBENGINE) {
        case 'MySQL':
            $query = "SELECT SUM_SEC_TO_TIME(duration) FROM track WHERE client_id = ?";
            break;
        case 'pgsql':
            $query = "
                SELECT sd.dur / 60 || ':' || TO_CHAR(sd.dur % 60, 'FM09')
                    FROM (
                        SELECT SUM(duration) AS dur FROM track WHERE client_details_id = ?
                    ) AS sd
            ";
            break;
    }
    if($query == '') return NULL;
    $sth = $dbh->prepare($query);
    $sth->execute(array($id));
    return $sth->fetchColumn();
}
*/
// utility functions

function set_vars( )
{
    global $CRUD;
    if(isset($CRUD["_BTN_ARRAY"])) foreach ( $CRUD["_BTN_ARRAY"] as $m ) $CRUD["BUTTONS"] .= $m;
    if(isset($CRUD["_HID_ARRAY"])) foreach ( $CRUD["_HID_ARRAY"] as $m ) $CRUD["HIDDENS"] .= $m;
    if(isset($CRUD["_MSG_ARRAY"])) foreach ( $CRUD["_MSG_ARRAY"] as $m ) $CRUD["MESSAGES"] .= $m;
    if(isset($CRUD["_ERR_ARRAY"])) foreach ( $CRUD["_ERR_ARRAY"] as $m ) $CRUD["ERRORS"] .= $m;
    if(isset($CRUD["_CON_ARRAY"])) foreach ( $CRUD["_CON_ARRAY"] as $m ) $CRUD["CONTENT"] .= $m;
    if(isset($CRUD["_PRE_ARRAY"])) foreach ( $CRUD["_PRE_ARRAY"] as $m ) $CRUD["PRECONTENT"] .= $m;
    if(isset($CRUD["_POST_ARRAY"])) foreach ( $CRUD["_POST_ARRAY"] as $m ) $CRUD["POSTCONTENT"] .= $m;
}

// make a field name display-friendly
function display_name ($n)
{
    // start with the exceptions
    if ($n == 'track_number') return 'Track';

    $n = strtr( $n, '_', ' ' );     // make _'s into spaces. 
    $n = ucwords( $n );
    return $n;
}

//
// shortcuts and setter-getters
//


// shortcut for reading field data from _REQUEST
function get_field ( $f )
{
    return stripslashes($_REQUEST[$f]);
}

// shortcuts for html elements

function heading ( $s )
{
    return "<p class=\"heading\">$s</p>\n";
}

function subheading ( $s )
{
    return "<p class=\"subheading\">$s</p>\n";
}

function start_form ( $name = "" )
{
    global $CRUD;
    $self = $CRUD['SELF'];

    if($name) $name = " name=\"$name\"";
    return "<form action=\"$self\" method=\"POST\"$name>\n";
}

function end_form ()
{
    return "</form>\n";
}

function track_input_text( $c, $n, $v )
{
    return "<input class=\"$c\" type=\"text\" name=\"$n\" value=\"$v\">";
}

function hidden_element( $n, $v )
{
    return "<input type=\"hidden\" name=\"$n\" value=\"$v\">";
}

function button_element( $n, $v )
{
    return "<input type=\"submit\" name=\"$n\" value=\"$v\">";
}

function action_button( $n, $v )
{
    return "<input class=\"action_button\" type=\"submit\" name=\"$n\" value=\"$v\">";
}

function javascript_focus( $form, $field )
{
    $a = "<script language=\"javascript\"> <!--\n";
    $a .= "  document.$form.$field.focus();\n";
    $a .= "// --> </script>\n";
    postcontent( $a );
}

// setter/getters

function form_head( $s )
{
    global $CRUD;
    if($s) $CRUD["FORM_HEAD"] = $s;
    return $CRUD["FORM_HEAD"];
}

// setters for display arrays

function button( $n, $v )
{
    global $CRUD;
    $CRUD["_BTN_ARRAY"][] = "<input class=\"submit_button\" type=\"submit\" name=\"$n\" value=\"$v\">\n";
}

function hidden( $n, $v )
{
    global $CRUD;
    $CRUD["_HID_ARRAY"][] = "<input type=\"hidden\" name=\"$n\" value=\"$v\">\n";
}

function content( $s )
{
    global $CRUD;
    $CRUD["_CON_ARRAY"][] = "\n<div class=\"content\">$s</div>\n";
}

function precontent( $s )
{
    global $CRUD;
    $CRUD["_PRE_ARRAY"][] = $s;
}

function postcontent( $s )
{
    global $CRUD;
    $CRUD["_POST_ARRAY"][] = $s;
}

// db_message( $dbh )
// get the error message element from the array
// (PHP functions aren't first-class, so foo()[n] doesn't work.)
function db_message($dbh)
{
    $e = $dbh->errorInfo();
    return $e[2];
    
}

function message()
{
    global $CRUD;
    $args = func_get_args();
    if(count($args) < 1) return;
    $s = vsprintf(array_shift($args), $args);
    $CRUD["_MSG_ARRAY"][] = "<p class=\"message\">$s</p>\n";
}

function error_message()
{
    global $CRUD;
    $args = func_get_args();
    if(count($args) > 0) {
        $s = vsprintf(array_shift($args), $args);
        $CRUD["_ERR_ARRAY"][] = "<p class=\"error_message\">$s</p>\n";
    }
    return isset($CRUD["_ERR_ARRAY"]) ? $CRUD["_ERR_ARRAY"] : array();
}

function error( $s )
{
    error_message($s);
    page('plain');
}

?>
